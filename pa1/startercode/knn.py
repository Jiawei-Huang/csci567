...
class KNN:
    ...
    def predict(self, features):
        ...
        for feature in features:
            ...
            label = np.bincount(np.array(k_neighbors).astype(int)).argmax()
            result.append(label)
    ...
    def get_k_neighbors(self, point):
        ...
        """
        # raise NotImplementedError
        dist = np.array([[self.distance_function(point, self.features[i]), self.labels[i]] for i in range(len(self.features))])
        dist_sort = dist[dist[:,0].argsort()]

        result = dist_sort[:self.k, 1].astype(int)

        # result = self.labels[np.argpartition(dist, -self.k)[-self.k:]]
